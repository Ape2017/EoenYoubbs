layui.define(['layer','element', 'fortree'], function(exports) {
     ...
     * 添加导航
     */
    function addNav(data, topid, idname, pidname, nodename, imgavatr, cassname, urlname) {
        topid    = topid    || 0;
        idname   = idname   || 'id';
        pidname  = pidname  || 'pid';
        imgavatr = imgavatr || 'avtid';
        nodename = nodename || 'node';
        cassname = cassname || 'cass';
        urlname  = urlname  || 'url';
        ...
        mytree.forcurr = function(v, k, hasChildren) {
            html += '<a href="javascript:;"'+(v[urlname] ? ' data-url="'+v[urlname]+'" data-id="'+v[idname]+'"' : '')+'><img src="/avatar/normal/'+v[imgavatr]+'.png" />';
            html += v[nodename];
            html += '</a>';
        };
